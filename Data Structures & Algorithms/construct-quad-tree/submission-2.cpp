...
public:
    ...
    Node* dfs(vector<vector<int>>& grid, int n , int r, int c) {
        ...
        // means its same 
        if (topLeft->isLeaf && topRight->isLeaf && bottomRight->isLeaf && bottomLeft->isLeaf && topLeft->val == topRight->val && topLeft->val == bottomRight->val && topLeft->val == bottomLeft->val) {
